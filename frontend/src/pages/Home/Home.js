...
const Home = () => {
  ...
  useEffect(()=>{
    ...
    tl.to([Lefta.current],{translateX:-5 ,rotate:4, duration:.1,  ease: "Expo".easeout },'+2')
    tl.to([Righta.current],{translateX:15,rotate:-4, duration:.1,ease:  "Expo".easeout },'+2')
    ...
    tl.to([Covera.current.children[0]],{translateY: '-90rem', duration: 5,
    oncomplete:()=>{
      const display= ()=>{
    ...
    tl.to([Covera.current.children[3]],{translateY: '20rem', duration: 1,
    oncomplete:()=>{
      const display= ()=>{
  ...
  //1
  tl.to(Text.lines, {ease: 'Power1'.easeout,autoAlpha: .1,translateY : -222,duration: 2, absolute: true,
   
  },'--=5')
 
  tl.to(textmain.current.children[0].children[1], {onStart:()=>{
    setTimeout(() => {
        firsta.current.style.display = 'none'
      }, 500);}
  , display: 'block',translateY : 0,duration: 2, autoAlpha:1, ease: 'Power4'.easeout},'--=4.8')

  //2
  tl.to(Text1.lines, {ease: 'Power1'.easeout,autoAlpha: .1,translateY : -222,duration: 2, absolute: true,
 
},'--=5')

tl.to(textmain.current.children[1].children[1], {onStart:()=>{
  setTimeout(() => {
...
//3
tl.to(Text2.lines, {ease: 'Power1'.easeout,autoAlpha: .1,translateY : -222,duration: 2, absolute: true,
 
},'--=5')

tl.to(textmain.current.children[2].children[1], {onStart:()=>{
setTimeout(() => {
    firsta2.current.style.display = 'none'
  }, 500);}
, display: 'block',translateY : 0,duration: 2, autoAlpha:1, ease: 'Power4'.easeout},'--=4.8')

 tl.fromTo([Overlaya.current],{width:'100vw',height:'0vh',clipPath: "polygon(0% 54%, 100% 53%, 100% 53%, 0% 54%)"},{height:'100vh',width:'100vw',
   duration: 1, onStart:()=>{
    setTimeout(() => {
    playa()
    }, 200);
    ...
    <Cover ref={Covera}>
        ...
        <Wrappermid ref={wrapper}>
           ...
           <Midtext ref={textmain}>
            <Overlay><h1 ref={firsta}>LA</h1> <h1>Shapping The</h1></Overlay>
            <Overlay1><Maison ref={firsta1}>MAISON</Maison> <Maison>Coffee</Maison> </Overlay1>
            <Overlay2><h1 ref={firsta2}>DEUZA</h1><h1>Culture</h1></Overlay2>
